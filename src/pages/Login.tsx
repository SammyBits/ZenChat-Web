...
import { loginAuthServices } from "../services/Auth/AuthServices";
import { useCookies } from "react-cookie";
export const Login = () => {
  const { setEmail, setPassword, email, password } = useLogin();
  const [cookies, setCookie] = useCookies(['auth-token']);

  const hourExpiration = 3600;

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const response = await loginAuthServices(email, password);
    setCookie('auth-token', response.result.token, { path: '/', maxAge: hourExpiration * 1000});
